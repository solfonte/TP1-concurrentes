...
use super::container_controller::{ContainerController, NormalContainerController, ContainerRechargerController, WaterNetworkController};
...
impl CoffeeMaker {
    ...
    fn turn_container_controllers_on(&self) -> Vec<JoinHandle<()>> {
        ...
        controller_handles.push(
            thread::spawn(move || {
                let coffee_controller = ContainerRechargerController::new(coffee_container_clone, grain_container_clone);
                coffee_controller.turn_on();
            }));

        let milk_foam_container_clone = self.ground_coffee_container.clone();
        let milk_container_clone = self.grain_container.clone();
        controller_handles.push(
            thread::spawn(move || {
                let foam_controller = ContainerRechargerController::new(milk_foam_container_clone, milk_container_clone);
    ...
    fn turn_dispensers_on(&self, order_queue_mutex: &Arc<Mutex<VecDeque<Order>>>, order_queue_semaphore: &Arc<Semaphore>) -> Vec<JoinHandle<u32>> {
